...
static struct ctimer co2_timer;
...
static void co2_sampling(){
    ...
    actual_co2 += increment;

    if( (actual_co2 > upper_bound_co2 && increment > 0 ) || 
        (actual_co2 < lower_bound_co2 && increment < 0)){
            increment *=-1;
         }

	sprintf(pub_topic, "%s", "co2");
	sprintf(app_buffer, "{ \"value\": %d }", actual_co2);
...
/*---------------------------------------------------------------------------*/
PROCESS(mqtt_client_process, "Carbon dioxid sensor MQTT");
            ...
            uint16_t chunk_len)
{
    ...
    LOG_INFO("CO2 publish handler called: Received on topic = %s, chunk = %s\n", topic, chunk);
}
/*---------------------------------------------------------------------------*/
static void
