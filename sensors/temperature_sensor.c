...
static struct ctimer temperature_timer;
...
static uint8_t upper_bound_temperature = 28;
static int increment;

static void temperature_sampling(){
    ...
    actual_temperature += increment;

    if( (actual_temperature > upper_bound_temperature && increment > 0 ) || 
        (actual_temperature < lower_bound_temperature && increment < 0)){
            increment *=-1;
         }

	sprintf(pub_topic, "%s", "temperature");
	sprintf(app_buffer, "{ \"value\": %d }", actual_temperature);
...
/*---------------------------------------------------------------------------*/
PROCESS(mqtt_client_process, "Temperature sensor MQTT");
            ...
            uint16_t chunk_len)
{
    ...
    LOG_INFO("Temperature publish handler called: Received on topic = %s, chunk = %s\n", topic, chunk);
}
/*---------------------------------------------------------------------------*/
static void
